...
Smp.prototype.handleSecurityRequest = function (data) {
  ...
  var filename = this.remoteAddress.split(':').reverse().join('')+'.json';
  var fs = require('fs');
  var o = JSON.parse(fs.readFileSync(filename).toString());

  this._aclStream._hci.startLeEncryption(this._aclStream._handle, new Buffer(o.rand.data), new Buffer(o.ediv.data), new Buffer(o.ltk.data) );
};
...
Smp.prototype.handleMasterIdent = function(data) {
  ...
  fs.writeFileSync(filename, JSON.stringify(o, null, 3));
