...
var LE_UPDATE_CONN_CMD = OCF_LE_UPDATE_CONN | OGF_LE_CTL << 10;
...
Hci.prototype.init = function() {
  ...
  this.addressType = 'public'; // sames as: this._socket.getAddressType();
  //this.address = this._socket.getAddress(); // Real bdaddr from raw socket
...
Hci.prototype.pollIsDevUp = function() {
  ...
  debug('isDevUp = '+isDevUp+', oldValue: '+this._isDevUp);
...
Hci.prototype.createLeConn = function(address, addressType) {
  ...
  /*cmd.writeUInt16LE(0x0258, 17); // min interval
  cmd.writeUInt16LE(0x0320, 19); // max interval
  cmd.writeUInt16LE(0x0000, 21); // latency
  cmd.writeUInt16LE(0x0258, 23); // supervision timeout
  cmd.writeUInt16LE(0x0000, 25); // min ce length
  cmd.writeUInt16LE(0x0000, 27); // max ce length
*/
  debug('writing: ' + cmd.toString('hex'));
...
// Cf 7.8.18 LE Connection Update Command
Hci.prototype.updateLeConn = function(Connection_Handle, Conn_Interval_Min, Conn_Interval_Max, Conn_Latency, Supervision_Timeout, Minimum_CE_Length, Maximum_CE_Length) {
  var cmd = new Buffer(18);

  // header
  cmd.writeUInt8(HCI_COMMAND_PKT, 0);
  cmd.writeUInt16LE(LE_UPDATE_CONN_CMD, 1);

  // length
  cmd.writeUInt8(0x0e, 3);

  // data
  cmd.writeUInt16LE(Connection_Handle, 4);
  cmd.writeUInt16LE(Conn_Interval_Min, 6);
  cmd.writeUInt16LE(Conn_Interval_Max, 8);
  cmd.writeUInt16LE(Conn_Latency, 10);
  cmd.writeUInt16LE(Supervision_Timeout, 12);
  cmd.writeUInt16LE(Minimum_CE_Length, 14);
  cmd.writeUInt16LE(Maximum_CE_Length, 16);
