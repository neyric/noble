...
var Gatt = function(address, aclStream) {
  ...
  this._aclStream.on('encryptChange', this.onAclStreamEncryptBinded);
...
Gatt.prototype.handleRequest = function(request) {
  ...
  switch(requestType) {
    ...
    case ATT_OP_MTU_REQ:
      debug('Handling ATT_OP_MTU_REQ');
      response = this.handleMtuRequest(request);
      break;*/

    case ATT_OP_FIND_INFO_REQ:
    debug('Handling ATT_OP_FIND_INFO_REQ');
      response = this.handleFindInfoRequest(request);
      break;

    case ATT_OP_FIND_BY_TYPE_REQ:
      debug('Handling ATT_OP_FIND_BY_TYPE_REQ');
      response = this.handleFindByTypeRequest(request);
      //response = this.errorResponse(requestType, 0x0000, ATT_ECODE_ATTR_NOT_FOUND);
      break;

    case ATT_OP_READ_BY_TYPE_REQ:
      debug('Handling ATT_OP_READ_BY_TYPE_REQ');
      response = this.handleReadByTypeRequest(request);
      break;

    case ATT_OP_READ_REQ:
    case ATT_OP_READ_BLOB_REQ:
      debug('Handling ATT_OP_READ_BLOB_REQ || ATT_OP_READ_REQ');
      response = this.handleReadOrReadBlobRequest(request);
      break;

    case ATT_OP_READ_BY_GROUP_REQ:
      debug('Handling ATT_OP_READ_BY_GROUP_REQ');
      response = this.handleReadByGroupRequest(request);
      break;

    case ATT_OP_WRITE_REQ:
    case ATT_OP_WRITE_CMD:
      debug('Handling ATT_OP_WRITE_REQ || ATT_OP_WRITE_CMD');
      response = this.handleWriteRequestOrCommand(request);
      break;

    case ATT_OP_HANDLE_CNF:
      debug('Handling ATT_OP_HANDLE_CNF');
      response = this.handleConfirmation(request);
      break;

    default:
      debug('Handling Other');
      console.log('NOT SUPPORTED !!!!!', requestType);
...
Gatt.prototype.handleReadByTypeRequest = function(request) {
  ...
  if ('2803' === uuid) {
    debug('handleReadByTypeRequest for uuid 2803');
    var characteristics = [];
    ...
    if (secure && !this._aclStream.encrypted) {
      ...
      debug('secure && !this._aclStream.encrypted');

      debug('sending error: ATT_OP_READ_BY_TYPE_REQ, ATT_ECODE_AUTHENTICATION');
      response = this.errorResponse(ATT_OP_READ_BY_TYPE_REQ, startHandle, ATT_ECODE_AUTHENTICATION);
    } else if (valueHandle) {

      debug('sending response: ATT_OP_READ_BY_TYPE_RESP, valueHandle = ', valueHandle);

      var data = this._handles[valueHandle].value;

      debug('sending response data', data);
      ...
      debug('sending response: ATT_OP_READ_BY_TYPE_REQ, ATT_ECODE_ATTR_NOT_FOUND');
...
Gatt.prototype.handleReadByGroupRequest = function(request) {
  ...
  debug('read by group: startHandle = 0x' + startHandle.toString(16) + ', endHandle = 0x' + endHandle.toString(16) + ', uuid = 0x' + uuid.toString(16) + ', en dec : '+uuid);
...
Gatt.prototype._queueCommand = function(buffer, callback, writeCallback) {
  ...
  console.log('Gatt current command', this._currentCommand);
